...
extern "C" {
    #include "../Dependencies/htslib-1.11/htslib/sam.h"
...
class BAMFile {
    ...
    public:
        friend std::unique_ptr<BAMFile> openBAMFile(const char *);
...
class PositionIterator {
    ...
    protected:
        ...
        std::unique_ptr<bam_mplp_s, decltype(&bam_mplp_destroy)> mplp_iter_;
        ...
        PositionIterator(
            std::unique_ptr<PileupData>,
            std::unique_ptr<bam_mplp_s, decltype(&bam_mplp_destroy)>,
...
std::unique_ptr<BAMFile> openBAMFile(const char*);
