...
BAMFile::BAMFile(
    std::unique_ptr<htsFile, decltype(&hts_close)> bam,
...
std::unique_ptr<BAMFile> openBAMFile(const char *file_name) {
    std::unique_ptr<htsFile, decltype(&hts_close)> bam(hts_open(file_name, "rb"), hts_close);
    if (!bam) throw std::runtime_error("Failed to open BAM file.");

    std::unique_ptr<hts_idx_t, decltype(&hts_idx_destroy)> idx(sam_index_load(bam.get(), file_name), hts_idx_destroy);
    if (!idx) throw std::runtime_error("Failed to open BAM index.");

    std::unique_ptr<bam_hdr_t, decltype(&bam_hdr_destroy)> header(sam_hdr_read(bam.get()), bam_hdr_destroy);
    if (!header) throw std::runtime_error("Failed to open BAM header.");
...
std::unique_ptr<PositionIterator> BAMFile::pileup(const std::string& region) {
    ...
    std::unique_ptr<bam_mplp_s, decltype(&bam_mplp_destroy)> mplp_iter(mplp, bam_mplp_destroy);
...
) : pileup_data_(std::move(pileup_data)),
    mplp_iter_(std::move(mplp_iter)),
    pileup_(std::move(pileup)),
...
std::unique_ptr<Position> PositionIterator::next() {
    if(!has_next()) throw std::runtime_error("No more positions to iterate.");
...
std::unique_ptr<Alignment> Position::next() {
    if (current_ >= count_) throw std::runtime_error("No more reads in position to iterate.");
